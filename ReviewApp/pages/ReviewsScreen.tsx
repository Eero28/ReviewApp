...
const ReviewsScreen: FC = () => {
  ...
  const filteredReviews = userReviews.filter((val) => {
    const searchResult = val.reviewname.toLowerCase().includes(searchTerm.toLowerCase());
    return searchResult;
  });
    ...
    <TouchableWithoutFeedback onPress={dismissKeyboard}>
      <SafeAreaView style={styles.container}>
        ...
        {filteredReviews.length === 0 && searchTerm ? (
          ...
          <FlatList
            data={filteredReviews.length <= 0 ? userReviews : filteredReviews}
            renderItem={({ item }: { item: ReviewItemIf }) => <ReviewItem item={item} />}
