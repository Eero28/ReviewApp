...
export const toggleLike = async (
    ...
    id_review: number,
    setLikesState: React.Dispatch<React.SetStateAction<{ user: UserInfo[]; isLiked: boolean }>>
) => {
    if (!userInfo) {
...
export const deleteLike = async (id_user: number, id_review: number, setLikesState: React.Dispatch<React.SetStateAction<{ user: UserInfo[]; isLiked: boolean }>>) => {
    try {
        if (!id_user) {
            throw new Error("User ID is missing");
        }
        await axios.delete(`${API_URL}/likes/unlike/review/${id_review}/user/${id_user}`);
...
export const getReviewLikes = async (id_user: number, id_review: number, setLikesState: React.Dispatch<React.SetStateAction<{ user: UserInfo[]; isLiked: boolean }>>) => {
    try {
        const response = await axios.get(`${API_URL}/likes/users/review/${id_review}`);
        const usersWhoLiked: UserInfo[] = response.data.data;
        const isLikedByUser = usersWhoLiked.some(like => like.id_user === id_user);
